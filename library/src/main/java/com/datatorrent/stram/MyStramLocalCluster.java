...
import com.datatorrent.stram.MyLocalMode.Controller;
...
public class MyStramLocalCluster implements Runnable, Controller
{
  private static final Logger LOG = LoggerFactory.getLogger(MyStramLocalCluster.class);
  // assumes execution as unit test
  private static final File DEFAULT_APP_DIR = new File("target", MyStramLocalCluster.class.getName());
  ...
  private MyServer bufferServer = null;
  ...
  private class LocalStreamingContainerLauncher implements Runnable
  {
    ...
    {
      LOG.error("Launching local streaming container - 1");
      ...
      LOG.error("Launching local streaming container - 2");
      this.child = new LocalStreamingContainer(containerId, umbilical, wingen);
      ContainerResource cr = new ContainerResource(cdr.container.getResourceRequestPriority(), containerId, "localhost",
          cdr.container.getRequiredMemoryMB(), cdr.container.getRequiredVCores(), null);
      LOG.error("Launching local streaming container - 3");
      StreamingContainerAgent sca = dnmgr.assignContainer(cr, perContainerBufferServer ? null : bufferServerAddress);
      LOG.error("Launching local streaming container - 4");
      if (sca != null) {
        childContainers.put(containerId, child);
        LOG.error("Launching local streaming container - 5");
        Thread launchThread = new Thread(this, containerId);
        containerThreads.add(launchThread);
        launchThread.start();
        LOG.error("Launching local streaming container - 6");
      }
    }
    ...
    public void run()
    {
      try {
        ...
        LOG.error("Started container {}", containerId);
        child.run(ctx);
  ...
  public MyStramLocalCluster(LogicalPlan dag) throws IOException, ClassNotFoundException
  {
    dag.validate();
  ...
  public MyStramLocalCluster(LogicalPlan dag, MockComponentFactory mcf) throws Exception
  {
    this(dag);
  ...
  /**
   * Return the container that has the given operator deployed. Returns null if
   * the specified operator is not deployed.
   *
   * @param planOperator
   * @return
   */
  ...
  {
    int count = 1;
    LOG.error("Starting run : 1");
    if (!perContainerBufferServer) {
      LOG.error("Starting run : 2");
      StreamingContainer.eventloop.start();
      LOG.error("Starting run : 3");
      bufferServer = new MyServer(StreamingContainer.eventloop, 0, 1024 * 1024, 8);
      LOG.error("Starting run : 4");
      try {
        bufferServer.setSpoolStorage(new DiskStorage());
        LOG.error("Starting run : 5");
      } catch (IOException e) {
        LOG.error("Starting run : 6");
        throw new RuntimeException(e);
      }
      bufferServerAddress = InetSocketAddress.createUnresolved(LOCALHOST, bufferServer.run().getPort());
      LOG.error("Starting run : {}", count++);
      LOG.error("Buffer server started: {}", bufferServerAddress);
    }

    long endMillis = System.currentTimeMillis() + runMillis;
    List<Thread> containerThreads = new LinkedList<>();
    LOG.error("endMillis : {}", endMillis);
    while (!appDone) {

      LOG.error("Starting appdone : 1");
      for (String containerIdStr : dnmgr.containerStopRequests.values()) {
        ...
        if (c != null) {
          ...
          LOG.error("Starting appdone : 2");
          c.processHeartbeatResponse(r);
          LOG.error("Starting appdone : 3");
        }
        dnmgr.containerStopRequests.remove(containerIdStr);
        LOG.error("Container {} restart.", containerIdStr);
        LOG.error("Starting appdone : 4");
        dnmgr.scheduleContainerRestart(containerIdStr);
        LOG.error("Starting appdone : 5");
      ...
      while (!dnmgr.containerStartRequests.isEmpty()) {
        ...
        LOG.error("Container Start Request {}" + cdr.container.getExternalId());
