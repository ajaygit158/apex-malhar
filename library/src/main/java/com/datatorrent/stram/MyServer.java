...
public class MyServer extends AbstractServer
{
   ...
   */
  public MyServer(EventLoop eventloop, int port)
  {
    this(eventloop, port, DEFAULT_BUFFER_SIZE, DEFAULT_NUMBER_OF_CACHED_BLOCKS);
  }

  public MyServer(EventLoop eventloop, int port, int blocksize, int numberOfCacheBlocks)
  {
  ...
  public void registered(SelectionKey key)
  {
    ...
    logger.error("Server started listening at {}", getServerAddress());
    latch.countDown();
    logger.error("MyServer --- Latch Down -- registered()");
    latch = null;
  }
  ...
  public void unregistered(SelectionKey key)
  {
    ...
    eventloop.submit(new Runnable()
    {
      ...
      public void run()
      {
        ...
        logger.error("Server stopped listening at {}", getServerAddress());
        latch.countDown();
        logger.error("MyServer --- unregistered -- latchdown");
        latch = null;
      }
    });
  }
  ...
  {
    logger.error("MyServer --- Initializing latch");
    final CountDownLatch latch = new CountDownLatch(1);
    this.latch = latch;
    eventloop.start(null, port, this);
    logger.error("Eventloop thread started");
    try {
      logger.error("Waiting for latch down");
      latch.await();
      logger.error("latch down successful");
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    new MyServer(eventloop, port).run();
    thread.join();
  }
        ...
        serverHelperExecutor.submit(new Runnable()
        {
          ...
          public void run()
          {
            try {
              ...
              logger.error("Buffer server {} failed to tear down subscriber {}.", MyServer.this, subscriber, t);
            }
          }
  ...
  {
    logger.error("MyServer - Exception : {}", el.toString(), cce);
